...
function factory(angular, $) {
	var namespace = "/setting/budget";
	/** 定义的service，会被注册为BudgetSubjectService */
	var service = function($rootScope, $http, $q, $timeout) {
		...
		return {
			...
			save: function(budgetSubject, ns) {
				var deferred = $q.defer();
				var $this = this;
				var module =","+ budgetSubject.modules+",";
				var roles = ","+budgetSubject.accountingRoles+",";
				budgetSubject = $.extend({ }, budgetSubject,{
					modules :module,
	...
	service.$name = "BudgetSubjectService"; // 可以省略，系统会默认以"模块名+Service"进行注册.如果此模块可能被其他模块依赖,则不能省略

	/** 定义的controller，会被注册为BudgetSubjectController */
	var controller = function($scope, $rootScope, $window, BudgetSubjectService, LocaleService) {
		...
		$.extend($scope, {
			...
			query: function(params, pageRequest) {
				...
				params = $.extend(params || { }, BudgetSubjectService.getCurrentParams() || { });
				BudgetSubjectService.query(params, pageRequest, session.currentMenu.namespace).then(function(page) {
					current.page = page;
				});
			...
			add: function() {
				...
				current.entity = $.extend({ isNew: true, status: app.status.valid, control:"1",adjust:"1",releaseType:"M"}, BudgetSubjectService.getCurrentParams());
			...
			edit: function(budgetSubject) {
				...
				if (budgetSubject.modules.substr(0,1)==',') 
				budgetSubject.modules=budgetSubject.modules.substr(1);//去掉第一位的逗号
				
				if (budgetSubject.accountingRoles.substr(0,1)==',') 
				budgetSubject.accountingRoles=budgetSubject.accountingRoles.substr(1);//去掉第一位的逗号
				
				budgetSubject.modulesArr = budgetSubject.modules.split(",");
				budgetSubject.accountingRolesArr = budgetSubject.accountingRoles.split(",");
				current.entity = $.extend({ }, budgetSubject,{
					modules : budgetSubject.modulesArr,
					accountingRoles : budgetSubject.accountingRolesArr
				});
			},
			...
			save: function(budgetSubject) {
				...
				BudgetSubjectService.save(budgetSubject || current.entity, session.currentMenu.namespace).then(function(entity) {
					if(entity.id != null){
			...
			remove: function(budgetSubject) {
				...
				BudgetSubjectService.remove(ids, session.currentMenu.namespace).then(function(budgetSubject) {
			...
			testCode: function(params) {
				...
				BudgetSubjectService.testCode(params,current.entity.code).then(function(list) {
					if(list.length == 0){
					}
						else{
	...
	controller.$inject = [ "$scope", "$rootScope", "$window", "BudgetSubjectService", "LocaleService"];
	controller.$name = "BudgetSubjectController"; // 可以省略，系统会默认以"模块名+Controller"进行注册
...
define("BudgetSubject", [ "angular", "jquery" ], function(angular, $) {
