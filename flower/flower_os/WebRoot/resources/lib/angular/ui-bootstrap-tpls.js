...
.constant('uibDatepickerPopupConfig', {
  altInputFormats: [],
  appendToBody: false,
  clearText: 'app.datepicker.clear',	// Clear
  closeOnDateSelection: true,
  closeText: 'app.datepicker.close',	// Done
  currentText: 'app.datepicker.today',	// Today
...
// 2016-04-20 罗刚 加入LocaleService以便对按钮指定i18n
.controller('UibDatepickerPopupController', ["LocaleService", '$scope', '$element', '$attrs', '$compile', '$log', '$parse', '$window', '$document', '$rootScope', '$uibPosition', 'dateFilter', 'uibDateParser', 'uibDatepickerPopupConfig', '$timeout', 'uibDatepickerConfig', 'uibDatepickerPopupAttributeWarning',
function(LocaleService, $scope, $element, $attrs, $compile, $log, $parse, $window, $document, $rootScope, $position, dateFilter, dateParser, datepickerPopupConfig, $timeout, datepickerConfig, datepickerPopupAttributeWarning) {
  var cache = {},
  ...
  this.init = function(_ngModel_) {
    ...
    // 2016-05-25罗刚 加入一个函数,以便将最初为String的数据转换为Date
    ngModel.$formatters.push(function(value) {
    	if (value && angular.isString(value)) {
    		var date;
    		if (dateFormat) {
    			date = dateParser.parse(value, dateFormat);
    		}
    		if (!date && Date.parse) date = Date.parse(value);
...
//    });
  ...
  $scope.getText = function(key) {
	// 2016-04-20 罗刚 对日期选择器的按钮加入i18n
	  var text = $scope[key + 'Text'] || datepickerPopupConfig[key + 'Text'];
	  return LocaleService.getText(text);
  };
  ...
  // 2016-04-20 罗刚 为element添加onclick事件,自动打开日期选择器
...
.service('uibDropdownService', ['$document', '$rootScope', function($document, $rootScope) {
  ...
  var closeDropdown = function(evt) {
    ...
    if (evt && openScope.getAutoClose() === 'outsideClick') {
        ...
        /* 2016-04-15 罗刚 修改源代码,使outsideClick时下拉菜单在点击element中的元素均不关闭 */
        var ele = openScope.getElement();
      if (ele && ele[0].contains(evt.target)) {
    	  return;
      }
    }
...
.directive('uibDropdownOpen', function() {	// 2016-04-25 罗刚 为下拉菜单增加uib-dropdown-open指令,添加此指令的元素,只要得到焦点,则会打开下拉菜单
  return {
    require: '?^uibDropdown',
    link: function(scope, element, attrs, dropdownCtrl) {
      if (!dropdownCtrl) {
