package org.wanwan.plugin.springjdbc;
...
import org.wanwan.plugin.springjdbc.support.SimpleBatchPreparedStatementSetter;
import org.wanwan.plugin.springjdbc.support.StatementSetter;
...
public class SpringDao {
	...
	public int[] updateList(List<Object> list) {
		...
		String[] realSqls = (String[]) ListUtil.cutArry(sqls, i);
		int[] rs = jdbc.batchUpdate(realSqls);
		System.out.println("更新记录:" + rs.length + "记录");
		return rs;
	}
	 ...
	 * 批量更新数据
	 * 
	 * @param sql insert into t_dictionary (id, code, name) values (?, ?, ?)
	 * @param list object[]对象
	 * @param setter 对object[]进行ps匹配
	 */
	public void updateList(String sql, List<Object[]> list, StatementSetter setter) {
		SimpleBatchPreparedStatementSetter simple = new SimpleBatchPreparedStatementSetter();
		simple.setList(list);
		simple.setSetter(setter);
		int[] rs = jdbc.batchUpdate(sql, simple);
