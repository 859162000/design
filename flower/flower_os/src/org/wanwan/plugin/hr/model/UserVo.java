...
public class UserVo implements java.io.Serializable, SqlVo {

	private int id; 
	private String code;//英文名 
	private String password = "4a7d1ed414474e4033ac29ccb8653d9b";//密码
	private String chinaName;//中文名 
	private String departmentcostcode;//部门名
	...
	public String toInsert(){
		String sql = "insert into t_user(id, code, name , dept_id, company_id, email,"
				+ "mobile, telephone, remarks, logon_password, sync_id) values('"
				+ this.id + "','" + this.code + "','" + this.toName()
				+ "','" + this.departmentcostcode + "','" + this.companyid + "','" 
				+ this.email + "','" + this.mobile + "','" + this.telephone+ "','" 
				+ this.remark + "','" + this.password + "','" + this.sync_id + "') ";
		return sql;
	}
	...
	public String toUpdate() {
		String sql = "update t_user set code='"+this.code+"',"
				+ "name='"+this.toName()+"',"
				+ "dept_id='"+this.departmentcostcode+"'"
				+ "company_id='"+this.companyid+"'"
				+ "email='"+this.email+"'"
				+ "mobile='"+this.mobile+"'"
				+ "telephone='"+this.telephone+"'"
				+ "remarks='"+this.remark+"'"
				+ "logon_password='"+this.password+"'"
				+ " where id=" + this.id;
		return sql;
	}
}
