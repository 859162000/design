...
/*****************************************************************************/
(function() {
	// 2016-04-05 罗刚 修改RSA.js的源码使之符合AMD规范
	
	var BigInt, BarrettMu;
...
return (result);
	...
	RSAAPP.RSAKeyPair = RSAKeyPair;				// 
	RSAAPP.encryptedString = encryptedString;	// 加密字符串
	RSAAPP.encode = encryptedString;	// 加密字符串
	RSAAPP.decryptedString = decryptedString;	// 解密字符串
	RSAAPP.decode = decryptedString;	// 加密字符串
	
	
	defined( "RSA", [ "BigInt", "Barrett" ], function(bi, bm) {
		BigInt = bi;
		BarrettMu = bm;
		BigInt.setMaxDigits(128);	// 需要128位的密钥
		return RSAAPP;
	});
})();
